<template>
    <AdminLayout>
        <div class="space-y-6">
            ...
            <!-- Loading State -->
            <div v-if="articleStore.isLoading && !articleStore.article" class="flex justify-center items-center py-12">
            ...
            <!-- Form -->
            <form v-if="articleStore.article" @submit.prevent="submitForm" class="space-y-6">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div v-if="currentFeaturedImage" class="mb-4">
...
const currentFeaturedImage = computed(() => articleStore.article?.featured_image_url);
...
const submitForm = async () => {
    ...
    const result = await articleStore.updateArticle(route.params.id, form);
...
// Watch for article changes to populate form
watch(() => articleStore.article, (newArticle) => {
    if (newArticle) {
        ...
        form.category_id = newArticle.category?.id || '';
        form.tags = newArticle.tags?.map(tag => tag.id) || [];
        form.status = newArticle.status || 'draft';
        form.scheduled_at = newArticle.scheduled_at ? newArticle.scheduled_at.slice(0, 16) : '';
        ...
        form.is_active = newArticle.is_active ?? 1;

        // Update editor content
        if (quillEditor.value) {
            quillEditor.value.setContents(form.content);
        }
    }
}, { immediate: true });
