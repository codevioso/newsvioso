<template>
    <AdminLayout>
        <div class="max-w-2xl mx-auto space-y-6">
            ...
            <!-- Loading State -->
            <div v-if="categoryStore.isLoading && !categoryStore.currentCategory" class="flex justify-center items-center py-12">
            ...
            <!-- Form -->
            <div v-if="categoryStore.currentCategory" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
                <form @submit.prevent="submitForm" class="p-6 space-y-6">
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        >
                            <span v-if="categoryStore.isLoading" class="flex items-center">
...
const populateForm = () => {
    if (categoryStore.currentCategory) {
        form.title = categoryStore.currentCategory.title;
        form.is_active = categoryStore.currentCategory.is_active;
    }
};
...
const submitForm = async () => {
    ...
    try {
        await categoryStore.updateCategory(route.params.id, form);
        
        successMessage.value = 'Category updated successfully!';
        
        // Redirect after a short delay
        setTimeout(() => {
            router.push('/secure/administrator/settings/categories');
        }, 2000);
        ...
        if (error.response?.data?.errors) {
            ...
            errorMessage.value = error.response?.data?.message || error.message || 'Failed to update category';
        }
    }
};
...
onMounted(async () => {
    try {
        ...
        console.error('Failed to initialize category edit:', error);
    }
});

// Watch for category data changes
watch(() => categoryStore.currentCategory, () => {
    if (categoryStore.currentCategory) {
        populateForm();
    }
});
